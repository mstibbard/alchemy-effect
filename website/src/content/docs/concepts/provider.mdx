...
> resource for write, `create`/`update` can assume it's safe to
> proceed. Don't repeat the ownership check inside `create` — that
> just hides the policy decision from the engine.
